# Agent notes — Evolver Cursor plugin
...
A plugin ships to users through Cursor, so "complete" includes the parts that are
easy to defer: the failure path when the engine is unreachable, the packaged build,
and the settings surface. A plugin that only works when everything is available is
not finished.
